...
def load_and_preprocess_data(file_path):
    try:
        # Load using row 5 as the header. Pandas will automatically handle the merged cells 
        # by generating column names like: "System Name / Ton", "System Name / SEER2"
        df = pd.read_excel(file_path, sheet_name="Sheet1", header=[4, 5])
        return df
...
if df_raw is not None:
    # Get all unique system categories from the top-level multi-index headers
    # Filtering out unnamed or empty columns
    system_types = sorted(list(set([col[0] for col in df_raw.columns if col[0] and not str(col[0]).startswith("Unnamed:")])))

    if system_types:
        st.sidebar.header("Filter Systems")
        selected_system = st.sidebar.selectbox("Select System Type", system_types)

        # Extract only the columns belonging to the selected system type
        # And flatten the multi-index header to just the clean metric names (e.g. "Ton", "SEER2")
        sys_cols = [col for col in df_raw.columns if col[0] == selected_system]
        
        if sys_cols:
            sys_df = pd.DataFrame()
            for col in sys_cols:
                metric_name = col[1].strip()
                sys_df[metric_name] = df_raw[col]

            # Locate the tonnage column
            ton_col_key = None
        ...
        st.error("No system types were successfully parsed from the Excel file headers. Please check row 5 and row 6 in your sheet.")
else:
